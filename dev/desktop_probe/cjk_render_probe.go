// Command cjk_render_probe renders ide_ref_cjk.html through wb-ui's
...
func main() {
	...
	wd, _ := os.Getwd()
	htmlPath := filepath.Join(wd, "cmd", "companion", "web-ui", "ide_ref_cjk.html")
	...
	wv.EnsureLayout()
	log.Printf("[PROBE] document=%v renderView=%v", wv.Document() != nil, wv.RenderView() != nil)
	...
	out := filepath.Join(wd, "dev", "desktop_probe", "cjk_wbui_shot.png")
	f, err := os.Create(out)
	if err != nil {
		log.Fatal(err)
	}
