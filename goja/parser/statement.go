...
func (self *_parser) parseForOf(idx file.Idx, into ast.ForInto, forAwait bool) *ast.ForOfStatement {
...
func (self *_parser) parseForOrForInStatement() ast.Statement {
		...
		if tok == token.VAR || tok == token.LET || tok == token.CONST {
			...
			if len(list) == 1 {
				if self.token == token.IN {
					if forAwait {
						self.error(idx, "for await...in is not allowed")
					}
					self.next() // in
					forIn = true
	...
	if forOf {
		return self.parseForOf(idx, into, forAwait)
	}
