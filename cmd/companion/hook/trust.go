...
// Trust gates project hooks. A project's .pair/settings.json can run
...
// root. The trust flag lives in user-global state (~/.pair/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.pair/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.pair.
...
// TrustPath is ~/.pair/trust.json (homeDir overrides ~).
