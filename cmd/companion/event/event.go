...
// show it" (streaming text in a chat panel, a card in a webview).
//
// The agent depends only on Sink; each frontend implements one. The chat bridge
// renders events to the streaming message; a headless run renders them to plain
// stdout; a future serve/websocket transports them to a remote client.
//
// Adapted from DeepSeek-Reasonix internal/event (typed event stream pattern).
package event
...
const (
	...
	ToolResult
	// UsageEvent carries per-turn token telemetry (Usage; Pricing optional, for cost).
	UsageEvent
	...
	// Summary so the placeholder still resolves.
	CompactionDone
	...
	// ToolResult for long tools like shell so a frontend can show live progress.
	ToolProgress
	...
	// injected as a user message. Text carries the raw steer content, so a
	// frontend can display it to the user as confirmation.
	Steer
)
...
// Usage carries per-turn token telemetry (Pricing optional, for cost).
type Usage struct {
	PromptTokens     int
...
// Pricing is a provider's per-1M-token rates, used to estimate spend.
type Pricing struct {
...
type Event struct {
	...
	Pricing          *Pricing     // Usage: for cost display (nil = omit cost)
