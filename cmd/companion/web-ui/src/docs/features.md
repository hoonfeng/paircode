...
- **关于面板** — 在"帮助 > 关于"中展示当前 IDE 版本号和系统信息，版本号由打包器（packager）通过编译注入，与二进制真实版本一致
- **更新日志页面** — 帮助文档中内置更新日志，完整记录每个版本的新增功能、改进和修复
- **版本号贯通** — 从 packager.json → companion.rc（Windows 文件版本资源）→ Go 二进制 → API 响应 → 前端显示，整条链路版本号同步，不会出现版本号对不上的情况
