...
//               + 资产/进化（asset_delete/evolution_save_capsule/save_gene/status）
//
// 合并说明（2026-09-04）：tool-verify 插件已并入本插件（验证对象同为
// .pair/memory 与 .pair/project-info 资产，与资源管理同源），JS 实现完全搬迁，
// 行为不变；tool-verify 插件目录已删除。
// 合并说明（2026-09-12 codex 精简轮）：tool-asset 插件已并入本插件
// （资产/进化与资源管理同域：智能资源总线的写入/维护动作），4 个工具声明
// 原样搬迁（execute 仍走 ctx.hostTool 宿主 Go 执行器）。
//
...
// 工具清单：resource_list、resource_search、resource_stats、memory_verify、project_info_verify、asset_delete、evolution_save_capsule、evolution_save_gene、evolution_status
// ═══════════════════════════════════════════════════════════════
const tools = [
  {
    "name": "resource_list",
  ...
  // ─── 资产/进化（2026-09-12 自 tool-asset 并入；tool-evolution 于 2026-09-04 已并入 tool-asset）───
  {
    "name": "asset_delete",
...
return {
  name: 'tool-resource',
  inject: ['fs'],
  purpose: '资源管理与资产/进化（resource_list/resource_search/resource_stats + memory_verify/project_info_verify + asset_delete/evolution_save_capsule/save_gene/status）（tool-verify 2026-09-04 已并入、tool-asset 2026-09-12 已并入）',
  apply(ctx) {
