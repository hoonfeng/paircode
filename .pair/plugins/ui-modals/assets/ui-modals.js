...
${h}`:h,l=l?`${l}
${g}`:g;let x=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(g,a,!0),this.lexer.state.top=x,n.length===0)break;let N=a.at(-1);if((N==null?void 0:N.type)==="code")break;if((N==null?void 0:N.type)==="blockquote"){let T=N,y=T.raw+`
`+n.join(`
`),S=this.blockquote(y);a[a.length-1]=S,o=o.substring(0,o.length-T.raw.length)+S.raw,l=l.substring(0,l.length-T.text.length)+S.text;break}else if((N==null?void 0:N.type)==="list"){let T=N,y=T.raw+`
`+n.join(`
`),S=this.list(y);a[a.length-1]=S,o=o.substring(0,o.length-N.raw.length)+S.raw,l=l.substring(0,l.length-T.raw.length)+S.raw,n=y.substring(a.at(-1).raw.length).split(`
...
`),header:[],align:[],rows:[]};if(n.length===o.length){for(let d of o)this.rules.other.tableAlignRight.test(d)?a.align.push("right"):this.rules.other.tableAlignCenter.test(d)?a.align.push("center"):this.rules.other.tableAlignLeft.test(d)?a.align.push("left"):a.align.push(null);for(let d=0;d<n.length;d++)a.header.push({text:n[d],tokens:this.lexer.inline(n[d]),header:!0,align:a.align[d]});for(let d of l)a.rows.push(Ge(d,a.header.length).map((i,h)=>({text:i,tokens:this.lexer.inline(i),header:!1,align:a.align[h]})));return a}}lheading(r){let t=this.rules.block.lheading.exec(r);if(t){let n=t[1].trim();return{type:"heading",raw:K(t[0],`
...
`)?"":`
`)+i.raw,g.text+=`
`+i.raw,this.inlineQueue.at(-1).src=g.text):this.tokens.links[i.tag]||(this.tokens.links[i.tag]={href:i.href,title:i.title},n.push(i));continue}if(i=this.tokenizer.table(t)){t=t.substring(i.raw.length),n.push(i);continue}if(i=this.tokenizer.lheading(t)){t=t.substring(i.raw.length),n.push(i);continue}let h=t;if((d=this.options.extensions)!=null&&d.startBlock){let g=1/0,x=t.slice(1),N;this.options.extensions.startBlock.forEach(T=>{N=T.call({lexer:this},x),typeof N=="number"&&N>=0&&(g=Math.min(g,N))}),g<1/0&&g>=0&&(h=t.substring(0,g+1))}if(this.state.top&&(i=this.tokenizer.paragraph(h))){let g=n.at(-1);o&&(g==null?void 0:g.type)==="paragraph"?(g.raw+=(g.raw.endsWith(`
`)?"":`
`)+i.raw,g.text+=`
