...
## 1.6.3 — 2026-09-17\r
\r
> 本版为创作域插件体系落地：六大创作域（画板 / UI 设计 / 3D 建模 / 音乐 / 2D 角色 / 人声）以\r
> 「一域一包」形态上线，新增独立发布插件渠道 \`plugins-dist/\`，建模内核补齐带孔挤出 / 倒角 /\r
> 圆角 / 扭转 / 扫掠与多边形 BSP 布尔路径（严格水密）。\r
\r
### 新增\r
\r
- **六大创作域插件（画板 / UI 设计 / 3D 模型 / 音乐 / 2D 角色 / 人声）** — 六个创作领域插件落地，每个领域一个插件包，含工具面（host 半：注册模型可直接调用的工具）+ 客户端面板（client 半）+ 前端资产。UI 与工具**同包**分发：面板不再作为独立插件包发布，\`ui-art\` / \`ui-design\` / \`ui-model\` / \`ui-music\` / \`ui-rig\` / \`ui-voice\` 的 UI 半已并入对应的 \`@paircode/tool-{art,design,model,music,rig,voice}\`（版本 \`0.2.0\`）；npm 上的旧 \`@paircode/ui-*\` 仍可安装，但已标记**废弃**（安装时提示「已并入 \`@paircode/tool-<x>\`」），请改用 \`tool-*\` 包。\r
- **独立发布插件渠道 \`plugins-dist/\`** — 创作域插件采用「随市场独立分发、不随 IDE 发版」的节奏，此前与 IDE 基线插件同处 \`.pair/plugins\` 无法区分。新增 \`plugins-dist/\` 作为独立发布插件真源（与 \`.pair/plugins\` 同权：区域发现、发布扫描、UI 构建都会扫描它，但**不进 IDE 发布包**），本地开发与验收以 junction 挂载（挂载态天然不进包）；新增护栏脚本 \`scripts/verify-dist-isolation.mjs\` 校验「独立发布包 ↔ 打包排除项」逐项一致。\r
- **主内容区视图 \`registerView\`（插件在主内容区开 tab）** — 插件可在主内容区 tab 栏开一个与对话 / 编辑器 / 市场 / 工具集同级的视图 tab（\`ui.registerView({id,title,icon,order,open,href,render})\`），与既有 \`registerPanel\`（藏在插件面板里的客户端面板）分工见 \`docs/plugin-development.md\` 新增对照表。挂载策略为**懒挂载 + 保持**：首次激活才渲染，切 tab 不卸载（保住 3D 视角 / 滚动位置等状态），关闭 tab 或卸载插件才清理，开启状态持久化；同时新增「并排对话」布局（主区分左右两栏：对话 + 当前视图，可换边）。\r
- **建模工具（tool-model）能力补齐**：\r
  - **2D 带孔轮廓挤出** — \`extrude\` 新增 \`holes\`（与 \`profile\` 同口径的孔轮廓数组），一步生成带孔板，不再需要 \`subtract\` 布尔（同尺寸挖 2 孔：276 面 / 4 ms，对比布尔 5763 面 / 218 ms）；洞方向统一、越界与相交明确报错，不静默降级；\`center\` 偏移对 circle / star / polygon 一致生效（偏心孔定位）。\r
  - **扭转挤出（\`twist\` + \`segments\`）与扫掠（\`type:"sweep"\` + \`along\`/\`closed\`/\`twist\`/\`scale\`/\`up\`）** — 按层刚体旋转，洞随外形同步转（带孔即内螺旋槽），每层扭转角 < 180° 校验防自交；扫掠框架用平行传输（Rodrigues，等价 RMF），规避 Frenet 在直线段无定义、拐点翻转导致的自交。\r
  - **倒角（chamfer）与圆角（fillet）** — 网格级边重建；凸体走 H-rep 精确构造（面平面 ∩ 斜面 / 球面半空间 → 三平面交点枚举 → 面内角度排序 → 扇形三角化 → 统一外向化），完全绕开布尔。同场景对照：H-rep 60 面 / 开边界 0 / χ=2，走 BSP intersect 则 1274 面 / 1274 条开边界 / χ=−126。\r
  - **圆角顶点混合（rolling-ball）** — 关键认识是换表示而非补面：圆角的几何本质是**形态学开运算**（先按球磨小再滚回），凸体可解析构造，于是立方体 12 棱全圆角不再依赖「逐边补面 + 顶点补片」（该路线实测开边界 206~554，顶点相交是死结）。\r
  - **多边形 BSP 布尔路径（I.6-2）并接线为默认** — BSP 全程保留凸多边形、导出前才三角化，共面分组天然免费（节点平面即其平面）。默认路径切换后**面数 −71%、M4 真缺口 −52.5%**；三角路径保留为回退 / 对照（\`setMeshBooleanLegacy(true)\` 或插件 \`config.legacyMeshBoolean=true\`），\`statsOut.path\` 回填实际所走路径。\r
  - **文件注册式实时预览** — 工具写出模型文件即登记并广播，面板列出工作区识别到的文件、点击当场预览（STL bin/ascii、OBJ、glTF、GLB），外部改动自动重绘（事件 + 定时复核）。\r
\r
### 修复\r
\r
- **布尔输出达成严格水密（M4 真缺口归零）** — 根因是各次布尔用各自的容差（随输入精度递增），同一条长边在相邻两侧的分割点跨次错开约 1~1.4 eps，任何焊接容差都合并不掉，严格半边上永远找不到配对。现按四项消解 T 缝：落在未配对边内部且贴近端点的顶点**吸附到端点**、真分割点**拓扑分割**插入未配对边、焊接比 0.5×→**1.5×eps**、**不劣化保护**（消解后若指标变差即回退）。结果：严格未配对 0 / 真缺口 0 / 缝总长 0 mm（此前 208 条真缺口），\`meshRepair\` 与 \`meshWatertightReport\` 同容差。踩坑已写入注释：T 缝插入必须**从该边的对顶点出发逐边分割并递归**（对整条边界链扇形分割会把「插入点 + 边两端点」三点共线拼成零面积三角形 → 非流形），插入点排序必须稳定（否则同向重复边）。\r
...
- **布尔真缺口根因修复（BSP 平面容差与量化坐标）** — 共面判定阈值 \`EPS_PLANE\` 固定为 1e-6，比本场景空间分辨率（meshEpsilon ≈ 3.5e-4）小 350 倍，把「近共面」误判为「跨平面」，同一几何平面被拆进多个 BSP 节点、在远处切出 1e-2 量级坐标偏差（反向三角形 11 → 0）。同时量化顶点表原按平面节点独立建立，板面与孔壁节点对「同一个几何顶点」各取各的代表坐标（边界错开约 1 格），改为跨节点共享；环边跨节点对齐继续收尾（三项修复后真缺口 893 → 726，零回退）。\r
- **Node 桥轨插件的面板在装载后被静默清掉** — 前端 \`plugin-runtime.js\` 的 \`syncClientHalves\` 在清理孤儿 client 半时会连带卸载由 boot 图（\`dsh.ui\` 区域包）装载的实例；声明了运行期 npm 依赖的 Node 桥轨插件（如 \`tool-voice\`）不出现在 \`/api/plugins\` 清单里，其 client 半只经 \`/api/ui-boot\` 下发，于是「人声」面板 / 视图在 boot 之后被静默移除。现按来源区分卸载对象。\r
- **多工作区 / 新会话下截图等产物落错盘** — 内嵌工具注册表原为「首次 root 永久缓存」的单例，第二个工作区或新会话仍复用第一个 root，导致 \`screenshot_stage\` / \`web_debug\` 等落盘工具的产物写进**旧工作区**（新工作区里找不到文件，被误判为「截图不落盘」）。现改为按 root 键控缓存（互斥保护），截图目录兜底绝对化。\r
\r
### 变更 / 改进\r
\r
- **插件版本与分发收口** — 六个创作域包统一 \`0.2.0\`；IDE 发布包的插件排除项由 11 项收敛为 6（独立发布包 = \`tool-{art,design,model,music,rig,voice}\`）；\`tool-voice\` 包名规范化为 \`tool-voice\`（原名带 scope 会让 \`/api/ui-boot\` 的入口 URL 带 \`@\`，导致 \`/plugins-assets\` 静态路由 404）。\r
- **旧 \`ui-*\` 引用清理** — 清除代码注释、工具提示、CI 示例、文档中残留的 \`ui-*\` 包引用（面板名改以 \`client.js\` 注册标题为准，如「ui-art 面板」→「**画板**」面板）；发布脚本的孤儿包废弃文案（\`ORPHAN_HINTS\`）补上六域映射，\`--deprecate-orphans\` 会给出「已并入 \`@paircode/tool-<x>\`（UI 与工具同包）」的准确说明。\r
- **建模面数口径更正** — 上一轮报告的面数对比取自不同焊接 eps 的诊断网格，两者不可比；同口径实测为 4416 → 4536 面（**+2.7%**，表面积变化 −4.7e-6%）。并给出结论：对已水密的网格再做共面合并是**以水密换面数**（4536 → 1187 面，但非流形 1 / 缝长 7.86，再修也回不来），两者同时追求必然振荡，故默认布尔路径不做共面合并（共面合并只保留在 legacy 对照分支）。\r
\r
### 文档\r
\r
- \`docs/creative-domain-landing-plan.md\`：落地形态改为「一域一包」，补五域推广与两条踩坑（scope 包名导致静态路由 404、boot 图与孤儿卸载冲突）；\`.pair/project.md\` 同步创作域插件目录关系。\r
- 应用内「更新日志 / API 文档」同步本版内容；\`/api/system/info\` 版本示例更新为 \`v1.6.3\`。\r
\r
---\r
\r
## 1.6.2 — 2026-09-13\r
\r
> 本版修复会话历史展示与上下文注入两类问题（\`v1.6.2\` 已于 2026-09-13 打 tag 并推送，更新日志条目随本版一并补齐）。\r
\r
### 修复\r
\r
...
`,1)[0],G;if(N=z,this.options.pedantic?(N=N.replace(this.rules.other.listReplaceNesting,"  "),G=N):G=N.replace(this.rules.other.tabCharGlobal,"    "),$.test(N)||R.test(N)||O.test(N)||W.test(N)||T.test(N)||C.test(N))break;if(G.search(this.rules.other.nonSpaceChar)>=y||!N.trim())g+=`
`+G.slice(y);else{if(B||x.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||$.test(x)||R.test(x)||C.test(x))break;g+=`
`+N}B=!N.trim(),h+=z+`
