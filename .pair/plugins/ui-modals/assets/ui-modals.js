...
`)?"":`
`)+i.raw,g.text+=`
`+i.raw,this.inlineQueue.at(-1).src=g.text):this.tokens.links[i.tag]||(this.tokens.links[i.tag]={href:i.href,title:i.title},n.push(i));continue}if(i=this.tokenizer.table(t)){t=t.substring(i.raw.length),n.push(i);continue}if(i=this.tokenizer.lheading(t)){t=t.substring(i.raw.length),n.push(i);continue}let h=t;if((d=this.options.extensions)!=null&&d.startBlock){let g=1/0,x=t.slice(1),N;this.options.extensions.startBlock.forEach(B=>{N=B.call({lexer:this},x),typeof N=="number"&&N>=0&&(g=Math.min(g,N))}),g<1/0&&g>=0&&(h=t.substring(0,g+1))}if(this.state.top&&(i=this.tokenizer.paragraph(h))){let g=n.at(-1);l&&(g==null?void 0:g.type)==="paragraph"?(g.raw+=(g.raw.endsWith(`
`)?"":`
`)+i.raw,g.text+=`
