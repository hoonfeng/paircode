var UiActivitybar = (function(exports, vue, uiState_js, pluginRuntime_js, appActions_js) {
  "use strict";
  ...
  const _hoisted_1 = { class: "activity-bar" };
  const _hoisted_2 = { class: "activity-top" };
  const _hoisted_3 = ["title", "onClick"];
  const _hoisted_4 = { class: "activity-bottom" };
  const _sfc_main = {
    __name: "ActivityBar",
    setup(__props) {
      const activitySlotEl = vue.ref(null);
      let activityUnsub = null;
      vue.onMounted(() => {
        activityUnsub = pluginRuntime_js.mountListSlot(activitySlotEl, "activitybar");
      });
      vue.onUnmounted(() => {
        if (activityUnsub) {
          activityUnsub();
      ...
      const isActive = (id) => {
        ...
        return uiState_js.state.activeActivity === id;
      };
      const switchIt = (id) => {
        appActions_js.switchActivity(id);
      };
      return (_ctx, _cache) => {
        return vue.openBlock(), vue.createElementBlock("div", _hoisted_1, [
          vue.createElementVNode("div", _hoisted_2, [
            (vue.openBlock(), vue.createElementBlock(
              vue.Fragment,
              null,
              vue.renderList(items, (item) => {
                return vue.createElementVNode("button", {
                  key: item.id,
                  class: vue.normalizeClass({ active: isActive(item.id), highlight: item.id === "chat" && vue.unref(uiState_js.state).rightPanelVisible }),
                  title: item.label,
                  onClick: ($event) => switchIt(item.id)
                }, [
            ...
            vue.createCommentVNode(" ★ activitybar 槽位（list 型）：插件在活动栏顶部叠加图标入口 "),
            vue.createElementVNode(
              "div",
              {
                ref_key: "activitySlotEl",
                ref: activitySlotEl,
                class: "plugin-slot-host plugin-slot-activitybar"
              },
              null,
              512
              /* NEED_PATCH */
  ...
  const ActivityBar = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-570982e5"]]);
  function mount(el) {
    const app = vue.createApp(ActivityBar);
    app.mount(el);
    return () => {
      app.unmount();
    };
  ...
  return exports;
})({}, window.__PAIRCODE_CORE.Vue, window.__PAIRCODE_CORE.uiState, window.__PAIRCODE_CORE.pluginRuntime, window.__PAIRCODE_CORE.actions);
