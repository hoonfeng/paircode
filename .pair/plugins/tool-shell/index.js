...
const tools = [
  {
    name: 'run_background',
    description: '在后台启动一条长命令，不阻塞 agent 循环（推荐用于 dev server、watch 模式、调试服务等）。返回进程 id，随后用 read_output 读输出、kill_process 停止。如果命令会长期运行或保持监听状态，优先用此工具。短查询请用 run_command。',
    usageGuide: '后台启动一条长命令，不阻塞 agent 循环。用于 dev server、npm run dev/watch 模式、调试服务、TCP 监听——这些场景只能用此工具，不可用 run_command。返回进程 id，之后用 read_output/kill_process 控制。',
    category: '执行',
    parameters: {
      type: 'object',
      properties: {
...
const impls = { run_background: runBackground, read_output: readOutput, kill_process: killProcess }

return {
  name: 'tool-shell',
  purpose: '后台进程工具（run_background/read_output/kill_process）——迁移自内置 registerShellTools，调用实现（JS 编排 ctx.process）完全在插件内',
  apply(ctx) {
