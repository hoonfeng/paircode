...
var (
	...
	cgBuildMu sync.Mutex                           // 增量构建互斥锁（全局串行，防重复全量读写）
...
// ★ 自动检测文件变更，需要时在后台触发增量构建（不阻塞查询/发送消息）。
func getCodeGraph(root string) (*codegraph.Graph, error) {
	...
	if e != nil && e.graph != nil {
		// ★ 每 30 秒检测一次源文件变更（检测+构建放后台 goroutine：
		//   增量构建需加载/保存数百 MB 图谱，同步执行会阻塞发送消息路径数秒~数十秒）
		cgCheckMu.Lock()
		needCheck := time.Since(e.lastCheck) > 30*time.Second
		if needCheck {
			e.lastCheck = time.Now()
		}
		cgCheckMu.Unlock()
		if needCheck {
			go tryIncrementalBuild(root)
		}
		cgEntriesMu.Lock()
...
// getCachedCodeGraph 只读缓存图谱：不触发变更检测、不触发构建。
// 供会话上下文注入等「仅需统计信息」的路径使用，避免发送消息时被重建阻塞。
// 缓存未命中时返回 nil（调用方自行降级）。
func getCachedCodeGraph(root string) (*codegraph.Graph, bool) {
	key := normRoot(root)
	cgEntriesMu.Lock()
	defer cgEntriesMu.Unlock()
	e := cgEntries[key]
...
// PruneCodeGraphs 清理已不在工作区中的项目图谱缓存
// （释放内存图引用；工作区移除后其缓存图不再存活）。
func PruneCodeGraphs(activeRoots []string) {
	keep := make(map[string]bool, len(activeRoots))
	for _, r := range activeRoots {
		keep[normRoot(r)] = true
	}
	cgEntriesMu.Lock()
	for k := range cgEntries {
		if !keep[k] {
			delete(cgEntries, k)
		}
	}
...
// needRebuild 轻量检测：检查是否有源文件比上次构建记录更新。
// 只扫描主要源目录（cmd/ internal/ pkg/），不反序列化图谱。
// 主项目共享 SQLiteStore 模式：检测 file_index 中记录的 mtime 与磁盘比较
// （有记录但无更新 → 无需构建；索引缺失 → 需初始构建）。
func needRebuild(root string) bool {
	idx, hasIndex := loadIndexForNeedRebuild(root)
	if hasIndex {
		return hasNewerGoFiles(root, idx)
	}
	return true // 索引缺失，需要初始构建
}

// loadIndexForNeedRebuild 按存储类型加载文件索引（用于轻量 mtime 检测）。
// 返回 (索引, 是否有索引)；共享 SQLite 模式读 file_index，其余读 index.json。
func loadIndexForNeedRebuild(root string) (map[string]time.Time, bool) {
	idx, err := cgStoreFor(root).LoadIndex()
...
// hasNewerGoFiles 轻量检测：主要源目录（cmd/ internal/ pkg/）下是否有 .go 文件
// 的 mtime 晚于索引记录（或不在索引中）。
func hasNewerGoFiles(root string, idx map[string]time.Time) bool {
...
// ★ 后台 goroutine 执行：调用方（getCodeGraph）已解耦，构建耗时不影响查询。
func tryIncrementalBuild(root string) {
	// ★ 构建互斥：增量构建加载/保存全量图谱（数百 MB），
	//   同一时间全局只允许一个构建，防止并发触发重复全量读写。
	cgBuildMu.Lock()
	defer cgBuildMu.Unlock()

	// ★ 工作区可能已被移除（后台构建启动到执行之间）：直接放弃，不写缓存。
	if !isActiveRoot(root) {
		return
	}
	...
	// ★ 构建完成后再次校验：工作区可能在构建期间被移除（或主项目切换），
	//   避免把已移除 root 的图塞回缓存（内存滞留）。
	if !isActiveRoot(root) {
		return
	}
