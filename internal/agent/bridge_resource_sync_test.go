...
// 真实应用会加载外置脚本而非 embed——若外置为旧版（无 cordis4/外部分支），
// 外部插件（runtime=dsh）装载必失败（旧脚本将 {spec,runtime} 对象 String() 化，
...
func TestBridgeNodeResourceSync(t *testing.T) {
	...
	for _, marker := range []string{`runtime=="dsh"`, "@deepseek-ai/cordis"} {
		if !strings.Contains(bridgeNodeJS, marker) {
			t.Fatalf("内嵌 bridge_node.js 缺少 外部分支标记 %q——运行时升级未落地", marker)
		}
	}
	...
	if string(data) != bridgeNodeJS {
		t.Fatalf("外置桥资源与内嵌版本不一致（外置 %d 字节 vs 内嵌 %d 字节）——请同步 internal/agent/bridge_node.js → .pair/assets/runtime/bridge_node.js，否则真实应用将运行旧版桥脚本导致 外部插件装载失败（F1a）", len(data), len(bridgeNodeJS))
	}
}
