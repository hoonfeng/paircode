...
// docState 记录上次发送给服务器的文档状态，以便 ensureSynced 通过 stat
// 检测出带外磁盘编辑（包括 bash 等任何工具修改了文件）。
...
// handleNotify 缓存诊断信息。版本号确保 waitDiagnostics 不会返回编辑前的内容。
...
// handleRequest 应答服务器→客户端的请求（某些服务器如 rust-analyzer
// 在初始化时会因缺少 workspace/configuration 回复而卡住）。
...
// waitDiagnostics 阻塞直到 publishDiagnostics（版本 >= minVer）到达或超时，
// 无论哪种情况都返回最新的缓存诊断。
...
// callRetry 在服务器返回 ContentModified (-32801) 时重试请求，
// 避免在重新索引过程中的短暂窗口导致错误。
