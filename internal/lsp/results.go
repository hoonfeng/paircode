...
// parseLocations 解码 textDocument/definition 和 /references 可能返回的三种形状：
// Location、Location[] 或 LocationLink[]。
...
// parseHover 解码 Hover.contents，它可以是 MarkupContent、单个 MarkedString 或数组。
...
// DocumentSymbol 表示一个符号（函数、类型、变量等）及其可能包含的子符号。
...
// parseDocumentSymbols 解析 documentSymbol 的响应。
// 响应可能是 []DocumentSymbol（嵌套）或 []SymbolInformation（扁平），统一转为 DocumentSymbol 列表。
func parseDocumentSymbols(raw json.RawMessage) []DocumentSymbol {
	...
	// 尝试以 SymbolInformation 格式解析（扁平结构，不含 children）
	var flat []struct {
		Name          string   `json:"name"`
		Kind          int      `json:"kind"`
		Detail        string   `json:"detail"`
