...
func TestNewManager(t *testing.T) {
	...
	if m.WsRoot() == "" {
		t.Error("WsRoot should not be empty")
	}
	m.Close()
}

func TestNewLSPTools(t *testing.T) {
	m := NewManager(t.TempDir(), nil)
	defer m.Close()
	...
	expectedNames := []string{"lsp_definition", "lsp_references", "lsp_hover", "lsp_diagnostics"}
	for i, name := range expectedNames {
		...
		if tools[i].Param == nil {
			t.Errorf("tool[%d] parameters should not be nil", i)
		}
	}
}

func TestExtractPosArgs(t *testing.T) {
	// Valid args
	args := map[string]any{
		"file":   "main.go",
		"line":   float64(42),
	...
	if file != "main.go" || line != 42 || symbol != "main" {
		t.Errorf("got %q %d %q", file, line, symbol)
	}

	// Missing line
	_, _, _, err = extractPosArgs(map[string]any{"file": "x.go", "symbol": "x"})
	if err == nil {
		t.Error("expected error for missing line")
	}

	// Empty file
	_, _, _, err = extractPosArgs(map[string]any{"file": "", "line": float64(1), "symbol": "x"})
	if err == nil {
		t.Error("expected error for empty file")
	}

	// Line < 1
	_, _, _, err = extractPosArgs(map[string]any{"file": "x.go", "line": float64(0), "symbol": "x"})
	if err == nil {
...
func TestFormatDiagnostics(t *testing.T) {
	empty := formatDiagnostics("test.go", nil)
	if empty != "no diagnostics for test.go" {
		t.Errorf("empty: %q", empty)
	}

	diags := []Diagnostic{
		{Range: Range{Start: Position{Line: 0, Character: 5}}, Severity: 1, Message: "syntax error", Source: "compiler"},
		{Range: Range{Start: Position{Line: 5, Character: 0}}, Severity: 2, Message: "unused var"},
	}
	result := formatDiagnostics("test.go", diags)
	if !strings.Contains(result, "2 diagnostic(s)") {
		t.Errorf("should contain count: %s", result)
	}
	if !strings.Contains(result, "1:6 error [compiler] syntax error") {
		t.Errorf("should format error: %s", result)
	}
	if !strings.Contains(result, "6:1 warning unused var") {
		t.Errorf("should format warning: %s", result)
	}
}

func TestSeverityNameDefault(t *testing.T) {
	if severityName[0] != "" {
		t.Errorf("unexpected severity 0 = %q", severityName[0])
	}
}
