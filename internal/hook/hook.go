...
// (.pair/settings.json, only when the project is trusted) and a global
// (~/.pair/settings.json) file. A hook's exit
...
//
// Ported from DeepSeek-Reasonix/internal/hook/ (Phase 2 upgrade).
package hook
...
// Events is every event, in a stable order — drives loading and /hooks listing.
var Events = []Event{
...
const (
	SettingsDirname  = ".pair"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.pair/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.pair/settings.json.
...
// hideWindow tries to hide the console window on Windows (optional).
// Overridden in hook_windows.go with a proper syscall; no-op elsewhere.
var hideWindow = func(*exec.Cmd) {}
