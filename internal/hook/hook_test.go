...
func TestDefaultSpawnerWindows(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows-specific test")
	}
	ctx := context.Background()
	// PowerShell echo on Windows
	r := DefaultSpawner(ctx, SpawnInput{Command: `echo hi`, Timeout: 5 * time.Second})
	if r.ExitCode != 0 {
		t.Errorf("expected exit 0, got %d (out=%q err=%v)", r.ExitCode, r.Stdout, r.SpawnErr)
	}
	// exit 2 via cmd
	r = DefaultSpawner(ctx, SpawnInput{Command: `cmd /c exit /b 2`, Timeout: 5 * time.Second})
	if r.ExitCode != 2 {
		t.Errorf("expected exit 2, got %d", r.ExitCode)
	}
	// timeout
	r = DefaultSpawner(ctx, SpawnInput{Command: `ping -n 30 127.0.0.1 >nul`, Timeout: 200 * time.Millisecond})
	if !r.TimedOut {
		t.Errorf("expected timeout, got %+v", r)
	}
}
