...
4. ~~**扭转挤出（twist）/ 扫掠（sweep）/ 圆角-倒角（fillet/chamfer）** 未做~~ → **twist/sweep 已完成**（见 I.8）；
   **chamfer 已完成**（凸体走 H-rep：解析精确 + 严格水密），**fillet 已完成到「互不相邻的棱」**（见 I.9）；
   剩余：圆角的**顶点混合**（相邻棱交会处的球面过渡面，rolling-ball）与**凹边填角**未做（均明确报错而非静默降级）；
   `polyhedron` 仍需手写顶点面表。
...
> 圆角取路线 B 的布尔近似，做到「互不相邻的棱完全水密」；**路线 C 的核心难点（顶点混合）仍未做**。
...
| 圆角：相邻棱（共享顶点） | ❌ **明确报错** | 需要 rolling-ball **顶点混合**（三条棱交会处的球面过渡面），本轮未做；硬做实测 boundary 206 |
| 凹边（内二面角 > 180°） | ❌ **明确报错** | 凹边倒角要「填」材料（另一套 union 逻辑）；默认只处理凸棱，诊断里如实报告跳过条数 |
| 顶点混合（三条以上棱交会处的过渡面） | ❌ 未做 | 圆角的顶点处是多个弧块的交集（近似），也是"多棱圆角"的前置 |

**下一步（未做）**：① rolling-ball **顶点混合** —— 打通后 fillet 就能支持"立方体 12 棱全圆角"；
② I.6-1 的**共面三角形合并** —— 能同时缓解 BSP 碎片、M4 的 T 缝与 fillet 的成本。
