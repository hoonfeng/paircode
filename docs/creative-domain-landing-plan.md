...
1. **BSP 碎片**（本域最大质量短板）：布尔结果面数偏大 —— 60×40×6 底板挖 4 个 ∅6 孔得 **5823 面**
   ...
   **共面三角形合并**（按平面分组 → 簇内边界环 → 单环重三角化 + 面积守恒校验，失败则回退），
   ...
   → **本轮已做**（见 §I.6-1）：干净簇可合并（单孔场景 **883 → 607 面 / −31%**），
     ...
     四个孔的主场景实测 3479 → 2975（−14.5%）。**根治已由 §I.6-2 完成** —— 多边形 BSP 路径把
     同一场景做到 768 面（−74.2%）、边界边 1451 → 928（见 §I.6-2）。
...
**下一步**：把 `meshBooleanPoly` 接成 `meshBoolean` 的默认路径（保留双路径开关与对照测试），
再补最小左转串环；之后 I.6-1 的三角域共面合并即可退役。
