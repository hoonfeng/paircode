...
> 验证对象：t2「实施 Round4：运行时完整化 + 外部插件直跑 + Go 核心处置」（commit 8f17896b 工作树 + t2 改动）。
> 结论：**FAILED（阻塞）** —— 发现 1 项**高严重度未覆盖缺陷（F1a：运行时资源外置桥脚本未同步）**，
> 导致真实应用（exe 置于仓库根、加载 `.pair/assets/runtime` 外置资源）下 外部插件装载失败；
...
TestToolRunCommand`（全部 msys bash 信号管道，沙箱禁 named-pipe 所致，外部文档化边界）
...
### Phase 1 —— 既有状态基线（patch 空、plugins.json 两条旧 cordis3 条目、未装 外部插件）

| 探活项 | 结果 |
|---|---|
| `/api/health` | ✅ 200 `{"status":"ok","workspace":"<仓库根>","folders":[3]}` |
...
### Phase 2-A —— 模拟 外部安装后状态（plugins.json 增 `{spec:"@nanmicoder/dsh-agent-teams@0.1.14",runtime:"dsh"}`；patch 增 runtime="dsh" 条目；npm 实装插件+peers 成功）

| 探活项 | 结果 |
|---|---|
| Node 桥启动 | ❌ **未启动**（bridge_err.log 零新行、应用日志零 `[node-bridge]` 行） |
| 外部插件装载 | ❌ 未装载（/api/commands 仍 owner=agent-teams，无第二条命令） |
...
| **外部条目** | ❌ **`插件 [object Object] 装载失败: ERR_MODULE_NOT_FOUND: Cannot find package '[object Object]'`** —— 部署桥脚本把对象条目 `String(spec)` 成 `[object Object]` |
...
readPluginSpecs/无 cordis4-外部分支；无任何同步脚本——仅 scripts/sync-web-dist.mjs 管 web dist），
...
| 外部服务面实测 | ✅ 装载期真实调用：`llm.current`、`agents.list`、`systemPrompt.section`、`commands.register`（`[node-bridge:diag]` 日志） |
...
| `POST /api/commands/run`（/agent-teams） | ✅ **端到端 cmdrun 往返成功**：HTTP → Go 宿主命令表 → 桥 cmdrun → 外部插件 handler → 返回 `{"kind":"error","text":"Usage: /agent-teams [--profile <name>] <goal>"}`（外部侧用法提示，证明 外部命令执行链路全通） |
| 工具注册 | ⚠️ **13 个 `agent_teams_*` 全部注册失败**：`工具 "agent_teams_*" 已被插件 agent-teams 注册，插件  不能覆盖`（claimTool 冲突；repo 移植版与 外部插件工具同名，二者为替代关系非并存关系；错误信息明确含处理建议，非静默） |
| 日志扫描 | ✅ 无 FATAL/panic |
...
## 4. 外部插件实跑验证（t1 验收标准对照）

| t1 验收标准 | 结果 | 证据 |
|---|---|---|
| `agent_teams_*` 13 工具注册进 Registry | ✅（隔离桥路径）/ ❌（真实应用并存 repo 移植版时，见 §3 Phase 2-D 冲突） | `TestNodeBridgeDSHPluginE2E` PASS：真实 npm 安装 dsh-agent-teams@0.1.14 + 外部 peers（cordis 4.0.1 / dsh-* rc.8 / schemastery）→ cordis4 装载 → **13 个 agent_teams_* 工具注册**（参考插件源码逐名核对 13 个：create/add_member/remove_member/reassign_task/create_task/resume/delete/claim_task/update_task/send_message/status/approve/edit_plan） |
...
| `/agent-teams` 命令注册 | ✅（隔离）/ ✅（真实应用 cmdrun 实跑） | 单元 `TestDSHBridgeSystemPromptCommand`（commands.register 进宿主表 + unregister 清理）PASS；真实应用 Phase 2-D `/api/commands/run` 实跑 外部 handler 成功 |
| 事件桥（agent/status 等订阅面） | ✅ | `TestBridgeEventSubscription`（白名单门控：未订阅零转发）PASS；`TestNodeBridgeAgentStatusEvent`（spawn → running 事件，载荷含 agent.id/session.header.cwd 对齐 外部 scheduler）PASS；Phase 2-D 实装载订阅日志 |
| 外部服务面（llm/agents/systemPrompt/commands） | ✅ | `TestDSHBridgeServices`（映射/错误路径/回落 mapBridgeService）PASS；Phase 2-D 实装载 diag 日志四服务真实调用 |
...
| agent-teams（repo 版）关键路径 | ✅ 升级前行为不变 | Phase 1 `/api/commands` + `/api/commands/run` 返回实时团队快照；卸载 外部状态还原后命令 owner 恢复 agent-teams |
...
| 工具名冲突语义（外部 vs repo 移植版并存） | ✅ 已自动化接管（遗留处置 2026-08-29） | 同名 13 工具由 `takeoverConflictingPlugin` 自动停用移植版并接管（同源判定：桥包短名去 dsh- 前缀 == 占用插件 id）；`/agent-teams` 命令按文档化语义由 外部覆盖；非同源冲突仍严格拒绝（明确报错） |
...
| ID | 级别 | 问题 | 建议处置 |
|---|---|---|---|
| **F1a** | **高（阻塞）** | 运行时资源外置优先（`LoadRuntimeAssetString` 外部优先 → embed 兜底），而 `.pair/assets/runtime/bridge_node.js`（12,329B，commit 2e0f36ab 起未更新）**无 Round4 改动**；真实应用桥脚本为旧版：plugins.json 对象条目解析为 `[object Object]`（`ERR_MODULE_NOT_FOUND`），cordis4/外部分支、subscribe/event/cmdrun 协议均不存在 → **外部插件在真实应用装载必然失败**；E2E 测试直写内嵌 `bridgeNodeJS` 绕过资源层，未覆盖此路径。t2 changedPaths 不含 .pair/assets（inScope 未列） | **阻塞修复**：将新版 bridge_node.js 同步到 `.pair/assets/runtime/bridge_node.js`（并核查 packager/release 资源同步链）；补一条「外置资源优先」场景的测试或文档化资源同步步骤（t6 范围外，需队长纳入 t6 或新修复任务） |
| F1b | 高（阻塞，与 t4 F1 同） | patch runtime="dsh" 不触发 `LoadCordisPatch` 桥启动（jsplugin.go:4003 只认 "node"）→ 外部插件跨重启丢失 | t6 修复已在途（工作树 jsplugin.go `rt=="node" \|\| rt=="dsh"` + TestLoadCordisPatchDSHRuntime）；修复后需重验重启场景 |
| F2 | 中（信息） | 外部插件与 repo agent-teams 移植版工具同名 13 项 → 并存时 外部工具全被拒（claimTool 明确报错，非静默）；`/agent-teams` 命令被覆盖（文档化语义） | 记录：两者为替代关系（安装 外部插件前先停 repo 移植版）；错误信息中插件名空白（`插件  不能覆盖`，bridge 工具归属名未填）可顺手补 |
...
- **通过项**：构建/静态检查/全量测试（14 项环境依赖失败与文档登记逐项吻合，零新增）；范围与依赖合规
  ...
  隔离桥路径下 外部插件实跑（13 工具 + create/status/delete + 落盘 + 事件桥 + 服务面）全通。
- **阻塞项**：**F1a**（外置桥资源未同步 → 真实应用 外部装载失败，t2/t4/E2E 均未覆盖）与 **F1b**
  （跨重启装载丢失，t4 F1 同源，t6 修复中）未解决前，验收标准「装载冒烟：外部插件装载成功」与
  「外部插件实跑」在**真实应用路径**不成立。
...
### 8.2 装载冒烟（临时端口 9548，模拟真实安装后状态：plugins.json 外部条目 runtime=dsh + patch runtime=dsh）

| 探活项 | 结果 |
|---|---|
| `/api/health` | ✅ 200 |
| **桥自动启动（F1b 复验）** | ✅ patch runtime="dsh" 即触发 `LoadCordisPatch` → Node 桥启动（旧版行为：只认 "node"，不启动） |
| **外部插件装载（F1a 复验）** | ✅ `已装载插件 @nanmicoder/dsh-agent-teams@0.1.14（runtime=dsh）`（cordis4 分支）；**零 `[object Object]` / ERR_MODULE_NOT_FOUND** |
| 外部服务面 | ✅ llm.current / agents.list / systemPrompt.section / commands.register 真实调用（diag 日志） |
...
| **cmdrun 往返（复验）** | ✅ `POST /api/commands/run` → Go 宿主命令表 → 桥 cmdrun → 外部 handler → `{"kind":"error","text":"Usage: /agent-teams [--profile <name>] <goal>"}` |
| 既有插件 | ✅ 46 插件 / 184 工具（=round3 基线）；2 条旧 cordis3 条目照常装载；13 个 外部工具与 repo 移植版同名被 claimTool 明确拒绝（F2 既有记录，替代关系，非静默、非 FATAL） |
...
`TestBridgeNodeResourceSync`（F1a 守卫：内嵌含 外部标记 + 外置与内嵌字节一致）✅ PASS、
...
真实应用（exe 仓库根）在真实安装状态（patch runtime=dsh）下：桥自动启动 → 外部插件 cordis4 分支装载成功 →
服务面/事件订阅/命令注册全部生效 → cmdrun 往返执行 外部 handler 成功，全程零 FATAL。
...
| 验收项 | 结果 |
|---|---|
| 前端构建 | ⚠️ 受限沙箱 `spawn EPERM` 阻断（vite/esbuild 子进程边界，round3 §10.1 同款）；**本轮前端零改动**，dist 为既有产物；沙箱外同一命令可执行（上报队长全权限通道） |
...
**结论**：Round4 核心目标「外部参考插件在升级后运行时直接装载运行」在真实应用
（pair.exe 仓库根启动）集成态下成立；与 repo 移植版并存时为文档化替代关系
（先停移植版再装 外部插件即可全量接管 13 工具）。可交付。
