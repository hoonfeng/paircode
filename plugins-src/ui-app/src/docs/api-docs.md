...
| callId | string | 否 | 提问的工具调用 ID（多窗口/排查用） |
| answers | array | 否 | 多问题回答 `[{id, answer}]`；与 `answer` 二选一（answers 优先） |

**响应：** `{"ok": true}`

**★ 2026-09-25 语义补充：提问已结束（超时/停止）后提交**

若提交时该会话已无 `ask_user` 在等待回答（提问超过 5 分钟超时、或会话已停止），
服务端不再把回答投递进内部通道（旧行为：投递成功但无人消费 → 回答静默丢失，
前端显示「已回答」而历史查无此答），而是**把回答落盘为一条会话 user 消息**
（内容为该回答原文；多问题形式为「（提问已超时结束，这是我对该提问的补充回答）」
+ 逐条 `[问题ID] 回答`），并回：

```json
{ "ok": true, "recorded": "message", "note": "该提问已结束（超时/停止），回答已记录为会话消息" }
```

同一回答重复提交（前端重试）是幂等的：与最近一条 user 消息文本相同时不重复落盘。
前端收到 `recorded=message` 时应提示用户「回答已记录为会话消息」并刷新会话历史。
