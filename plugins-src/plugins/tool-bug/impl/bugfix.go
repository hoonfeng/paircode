...
import (
	...
	"context"
	. "github.com/hoonfeng/paircode/plugins-src/plugins/tool-bug/toolbin"
)
...
func BuildDetailedFixPrompt(detected *BugDetectResult, root string) string {
	...
	// 修复指南
	b.WriteString("## 修复指南\n\n")
	b.WriteString("1. 使用 `read` 读取每个错误位置附近的代码\n")
	b.WriteString("2. 分析错误原因：是语法错误、类型不匹配、未定义标识符还是其他问题\n")
	b.WriteString("3. 使用 `edit` 或 `write` 修复\n")
...
func registerBugFix(r *Registry, root string) {
...
// Register 注册全部 BUG 检测与修复工具（独立二进制入口）。
func Register(r *Registry, root string) {
	registerBugDetect(r, root)
	registerBugFix(r, root)
}
